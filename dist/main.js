...
const db = new better_sqlite3_1.default(path_1.default.join(dbDir, "nook.sqlite"));
...
function init() {
    ...
    if (!totalUsers.total) {
        db.prepare("INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, ?)")
            .run("Administrador Nook", "admin@nook.com", bcryptjs_1.default.hashSync("admin123", 10), "admin");
        db.prepare("INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, ?)")
            .run("Cliente Demo", "cliente@nook.com", bcryptjs_1.default.hashSync("cliente123", 10), "cliente");
...
init();
function flash(request, msg, type = "ok") {
...
app.post("/login", (request, response) => {
    const user = db.prepare("SELECT * FROM users WHERE email = ?").get(request.body.email);
...
app.post("/reservas/:id", logged, (request, response) => {
    const livro = db.prepare("SELECT * FROM livros WHERE id = ?").get(request.params.id);
...
app.get("/minhas-reservas", logged, (request, response) => {
...
app.post("/reservas/:id/cancelar", logged, (request, response) => {
    const reserva = db.prepare("SELECT * FROM reservas WHERE id = ? AND user_id = ?").get(request.params.id, request.session.user.id);
...
app.post("/eventos/:id/participar", logged, (request, response) => {
...
app.get("/admin", adminOnly, (_, response) => {
    const stats = {
        livros: db.prepare("SELECT COUNT(*) AS total FROM livros").get().total,
        eventos: db.prepare("SELECT COUNT(*) AS total FROM eventos").get().total,
        clientes: db.prepare("SELECT COUNT(*) AS total FROM clientes").get().total,
        reservas: db.prepare("SELECT COUNT(*) AS total FROM reservas").get().total
    };
...
function crud(name, table, fields, uploadField) {
    ...
    app.post(`/admin/${name}`, adminOnly, upload.single(uploadField || "image"), (request, response) => {
    ...
    app.put(`/admin/${name}/:id`, adminOnly, upload.single(uploadField || "image"), (request, response) => {
        const current = db.prepare(`SELECT * FROM ${table} WHERE id = ?`).get(request.params.id);
        const data = fields.map((field) => {
            if (request.file && field.name === uploadField) {
                return request.file.filename;
            }
    ...
    app.delete(`/admin/${name}/:id`, adminOnly, (request, response) => {
